   ...
   <table border="1">
				  ...
				  <tbody>
				    <tr v-for="(item,key,index) in eqss" :value='item.id' style="font-size:18px;margin:15px 25px 1px;"   @click="getDataId(item.id)">
				      ...
				      <td style="color: darkgray;"><span @click="getAppinfo(item.id)">点击查看详情</span ></td>
				    </tr>
  </tbody>
</table>
  ...
  export default {
   ...
   mounted:function(){
    ...
    methods: {
      getDp: function () {
      	var id="";
        axios.post('/api/ospworkorder/opsworkorder/interface/workorder?state=1&id='+id).then(response => {
