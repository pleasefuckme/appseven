  ...
  <div class="container bgF">
    ...
    <ul class="formCom form-login form-mini mt10">
      ...
      <li :class='showPwd?"icon-eye1":"icon-eye2"'>
        <label>
          <span>设置密码</span>
          <input :type="showPwd?'password':'text'" placeholder="请输入密码"v-model.trim='pwd'> <!--placeholder="请输入6-12位数字与字母组合密码"--> 
...
//      if (checkPhone.test(this.phone) === false) this.$dialog('手机格式不正确');
        if (checkPhone.test(this.phone) === false) this.$dialog('账号格式不正确');
