  ...
  <div class="container bgF">
    ...
    <ul class="formCom form-login">
      <li>
        <label>
          <span>账号</span>
          <input type="text" placeholder="请输入账号" v-model.text='phone'/>
        </label>
      </li>
      <li>
      ...
      <li :class="showPwdOne?'icon-eye1':'icon-eye2'">
        <label>
          <span>登录密码</span>
          <input :type="showPwdOne?'password':'text'" placeholder="请输入5-12位密码" v-model.trim='pwdOne'>
      ...
      <li :class="showPwdTwo?'icon-eye1':'icon-eye2'">
        <label>
          <span>确认密码</span>
          <input :type="showPwdTwo?'password':'text'" placeholder="请输入5-12位确认密码" v-model.trim='pwdTwo'>
...
//      let checkPhone = /^[1][3578][0-9]{9}$/,
        let checkPhone = /^[\d\D]{5,12}$/,
          checkPwdOne = /^[\d\D]{5,12}$/,
          checkSMS = /^[0-9]{4,8}$/;
        //先做一些简单的判断再提交ajax
        if (this.checked === false) this.$dialog('请阅读并同意协议');
        else if (checkPhone.test(this.phone) === false) this.$dialog('账号格式不正确');
